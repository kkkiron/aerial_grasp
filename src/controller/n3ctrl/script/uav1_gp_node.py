...
class GP_Algorithm():
    def __init__(self):
        ...
        self.gp_pub = rospy.Publisher('uav1/n3ctrl/cmd_gp_out_a', gp_output, queue_size = 10)    #发布gp后的位置

        rospy.Subscriber("uav1/n3ctrl/ctrl_dbg/value", ControllerDebug, self.ctrl_callback)    #订阅期望控制量 roll pitch yaw thrust
        rospy.Subscriber("uav1/odom", Odometry, self.odom_callback)   #订阅odom 位置 
    ...
    def algorithm(self):
        ...
        # msg.gp_ay = 0.02
        # msg.gp_az = 0.02
        msg.gp_ax = model_err_gp[0]   #gp_output_a_x
        msg.gp_ay = model_err_gp[1]
        msg.gp_az = model_err_gp[2]

        msg.mse_gp_ax = model_mse_gp[0]
        msg.mse_gp_ay = model_mse_gp[1]
        msg.mse_gp_az = model_mse_gp[2]
        ...
        print('mse_gp_ax ', msg.mse_gp_ax)
        print('mse_gp_ay ', msg.mse_gp_ay)
        print('mse_gp_az ', msg.mse_gp_az)
        #发布消息
        # print('msg:', msg)
        ...
        model_err = np.array([self.a_x, self.a_y, self.a_z - g]) - model_pred  #实际 - 期望
        file = open('model_error.txt',mode = 'a',encoding='utf-8')
        file.write(str(model_err)+'\n')
        # print(model_pred)
        # print('***************************')
        # print(model_err)
