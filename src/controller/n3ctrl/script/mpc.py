...
class MPC():
    ...
    def update_params(self, dt, stepsize, N):
        ...
        # for smooth of u
        self.D_3 = np.zeros((self.udim * (self.N - 1), self.udim * self.N))
        for i in range(self.udim * (self.N - 1)):
            self.D_3[i, i] = -1
            self.D_3[i, i+self.udim] = 1
        self.S = np.eye(self.udim * (self.N - 1)) * 0.5
        self.S1 = np.matmul(np.matmul(self.D_3.T, self.S), self.D_3)
        ...
        self.P = np.matmul(np.matmul(self.B_all.T, self.Q_mpc), self.B_all) + self.S1
    ...
    def get_control(self, z_0, ref_traj):
        ...
        #     plt.show()
        # if self.N > 1:
        #     z_n2 = np.matmul(self.A1, z_next[:]) + np.matmul(self.B1, self.res.x[self.udim:2*self.udim]).reshape((self.xdim,1))
        #     z_n2_dot = (z_n2 - z_0) / dt / stepsize / 2
        #     return z_n2, z_n2_dot
