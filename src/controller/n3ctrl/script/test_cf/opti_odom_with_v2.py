    ...
    def receiveRigidBodyFrame(self,rigidBodyList, timestamp):
        ...
        for rigidBody in rigidBodyList:
            ...
            if len(self.l_odom[id]) == self.sample_interval:
                ...
                # msg2.deltaTime = deltatime
                #rospy.loginfo(deltatime)
                msg2.twist.twist.linear.x = (msg2.pose.pose.position.x - last_msg.pose.pose.position.x)/deltatime
                msg2.twist.twist.linear.y = (msg2.pose.pose.position.y - last_msg.pose.pose.position.y)/deltatime
                msg2.twist.twist.linear.z = (msg2.pose.pose.position.z - last_msg.pose.pose.position.z)/deltatime
                if abs(msg.linear.x) < 0.0001:
                   msg.linear.x = 0
                if abs(msg.linear.y) < 0.0001:
                   msg.linear.y = 0
                msg2.twist.twist.angular.x = self.ang_diff(angle[0],self.lastAngle[0])/deltatime
                msg2.twist.twist.angular.y = self.ang_diff(angle[1],self.lastAngle[1])/deltatime
                msg2.twist.twist.angular.z = self.ang_diff(angle[2],self.lastAngle[2])/deltatime
            ...
            self.l_odom[id][self.l_index[id]] = msg2
