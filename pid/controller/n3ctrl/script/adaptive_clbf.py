...
class AdaptiveClbf(object):
    def __init__(self, use_service = True, use_mpc=False, use_nmpc=False, use_trigger=False, use_IGPR=False):
        ...
        if self.use_service:
            ## train model action
            self.train_model_action_client = actionlib.SimpleActionClient('train_model_service', n3ctrl.msg.TrainModelAction)
            self.train_model_action_client.wait_for_server()
            self.train_model_goal = n3ctrl.msg.TrainModelGoal()
            ...
            if not use_trigger:
                if not use_IGPR:
                    ...
                    self.model = ModelIGPR2Service(self.xdim,use_service=False)
            else:
                if not use_IGPR:
                    from model_service import ModelGPService
                    self.model = ModelGPService(self.xdim,use_service=False)
    ...
    def update_params(self,params):
        ...
        self.dyn.m = self.params['mass']
        self.dyn.ga = self.params['g']
        # todo
        ...
        if self.use_mpc:
            self.mpc.update_params(self.params["mpc_dt"], self.params["mpc_stepsize"], self.params["mpc_N"])
    ...
    def get_control(self,z,z_ref,z_ref_dot,dt,use_model=False,add_data=True,check_model=True,use_qp=True,ref_traj=None,true_acc=[]):
        ...
        if self.use_mpc:
            ...
            # _nouse, self.z_ref_dot = self.mpc.get_control(z_0=self.z, ref_traj=ref_traj)
        ...
        if true_acc == []:
            self.ynew = (self.z[3:,:] -self.z_prev[3:,:]) / dt - mu_model
        else:
            self.ynew = true_acc
        if add_data:
            if self.use_service:
                try:
                    self.model_add_data_srv(self.z_prev.flatten(),self.ynew.flatten())
                except:
                    print("add data service unavailable")
            else:
                req = AddData2Model()
                req.x = self.z_prev.flatten()
                req.ynew = self.ynew.flatten()
        ...
        if check_model and self.model_trained:
            ...
            if predict_service_success:
                ...
                # print('yout', self.y_out)
                # print('zdot', self.z_dot)
